...
class NetworkScanner:
    def __init__(self, target: str, timeout: int = 10, max_threads: int = 10):
        self.target = self._validate_target(target)
        self.timeout = min(timeout, 30)  # Cap timeout at 30 seconds
        self.max_threads = min(max_threads, 20)  # Cap threads at 20
        self.results_cache = {}
    ...
    def _validate_target(self, target: str) -> str:
        ...
        try:
            ...
            # Not an IP address, try to validate as domain
            if not re.match(r'^[a-zA-Z0-9]([a-zA-Z0-9-]{0,61}[a-zA-Z0-9])?(\.[a-zA-Z]{2,})+$', target):
                raise ValueError("Invalid target format. Must be a valid IP address or domain name")
            return target
    ...
    def _is_rate_limited(self, scan_type: str) -> bool:
        """Check if scan type is currently rate limited."""
        cache_key = f"rate_limit_{scan_type}_{self.target}"
        last_scan = self.results_cache.get(cache_key)
        if last_scan:
            time_diff = time.time() - last_scan['timestamp']
            if time_diff < self.scan_config.get(f'{scan_type}_cooldown', 60):
                return True
        return False

    async def perform_scan(self, scan_type: str, **kwargs) -> Dict[str, Any]:
        """Perform network scan with caching and rate limiting."""
        if self._is_rate_limited(scan_type):
            raise ValueError(f"Rate limit exceeded for {scan_type} scan")

        cache_key = f"{scan_type}_{self.target}_{json.dumps(kwargs, sort_keys=True)}"
        cached_result = self._get_cached_result(cache_key)
        if cached_result:
            return cached_result

        try:
            scan_method = getattr(self, f"_scan_{scan_type}")
            result = await scan_method(**kwargs)
    ...
    async def _scan_ports(self, ports: str = None, scan_type: str = 'SYN') -> Dict[str, Any]:
        """Perform port scanning with improved reliability."""
        if not ports:
            ports = self.scan_config['port_ranges']['quick']

        try:
            nm = nmap.PortScanner()
            scan_args = f"-v -n -Pn -{scan_type.lower()[0]}S"
            
            # Add timing template based on scan scope
            if len(ports.split(',')[0].split('-')) > 1:
                start, end = map(int, ports.split(',')[0].split('-'))
                if end - start > 1000:
                    scan_args += " -T4"
                else:
                    scan_args += " -T3"
            
            self.logger.info(f"Starting port scan on {self.target} with args: {scan_args}")
            nm.scan(hosts=self.target, ports=ports, arguments=scan_args)
    ...
    def _parse_nmap_results(self, scanner: nmap.PortScanner) -> Dict[str, Any]:
        """Parse and format Nmap scan results."""
        results = {
            'target': self.target,
        ...
        for host in scanner.all_hosts():
            ...
            for proto in scanner[host].all_protocols():
                ...
                for port in ports:
                    ...
                    protocol_info.append(port_info)
                
                host_info['protocols'][proto] = protocol_info
    ...
    async def _scan_service(self, port: int) -> Dict[str, Any]:
        """Perform service detection on a specific port."""
        try:
            service_info = {
                'port': port,
                'service': None,
            ...
            nm = nmap.PortScanner()
            nm.scan(self.target, str(port), arguments="-sV -Pn")
            
            if self.target in nm.all_hosts():
                for proto in nm[self.target].all_protocols():
                    if port in nm[self.target][proto]:
                        service = nm[self.target][proto][port]
                        service_info.update({
                            'service': service.get('name'),
                            'version': service.get('version'),
                            'product': service.get('product')
            ...
            self.logger.error(f"Service scan failed for port {port}: {str(e)}")
            raise

    async def _grab_banner(self, port: int) -> Optional[str]:
        """Safely grab service banner from a port."""
        try:
